...
def train_network_on_top_of_other(model, train_loader, val_loader, optimizer, 
    ...
    writer = SummaryWriter('runs/pretrained_shape_espnet_one_output/')
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for imgs, masks in tqdm(data_loader):
                ...
                empty_channel = np.zeros((512, 512), dtype=np.uint64)
    ...
    torch.save(best_model_weights, 
        os.path.join(path_to_save_best_weights, 'pretrained_shape_espnet_sum_output{:2f}.pth'.format(best_val_loss)))
